...
public class ClickerGameUI : MonoBehaviour
{
    ...
    private void OnGUI()
    {
        ...
        GUILayout.Label("[ 직관적 조합 클리커 - 빌드 안정화 버전 ]", GUILayout.Width(500));
        ...
        string wRecStr = s.Upgrades.HasWoodRec ? "O" : "X";
        string pRecStr = s.Upgrades.HasWoodPickRec ? "O" : "X";
        GUILayout.Label($"[레시피] 목재({woodRecChance}%) [{wRecStr}] / 나무곡괭이({woodPickRecChance}%) [{pRecStr}]", new GUIStyle(GUI.skin.label) { fontSize = 13 });
        GUILayout.Space(5);

        if (GUILayout.Button($"숲 채집하기", GUILayout.Height(60)))
        {
        ...
        if (s.Upgrades.HasWoodPickRec && !s.Upgrades.HasWoodPick) DrawCraftButton("나무 곡괭이 제작 [가공된 목재(5)]", GameLogic.CanCraftWoodPick(s), () => { s = GameLogic.CraftWoodPick(s); ShowNotification("광산 해금!"); });
        if (s.Upgrades.HasShearsRec && !s.Upgrades.HasShears) DrawCraftButton("원예 가위 제작 [철(5) + 통나무(30)]", GameLogic.CanCraftShears(s), () => { s = GameLogic.CraftShears(s); ShowNotification("꽃 해금!"); });
        ...
        if (s.Upgrades.HasWoodPick)
        {
            ...
            string aRecStr = s.Upgrades.HasAlloyRec ? "O" : "X";
            string iRecStr = s.Upgrades.HasIronPickRec ? "O" : "X";
            string sRecStr = s.Upgrades.HasShearsRec ? "O" : "X";
            ...
            if (GUILayout.Button($"광산 채집하기", GUILayout.Height(60)))
            {
            ...
            if (s.Upgrades.HasIronPickRec && !s.Upgrades.HasIronPick) DrawCraftButton("철 곡괭이 제작 [나무 곡괭이 + 철(10)]", GameLogic.CanCraftIronPick(s), () => { s = GameLogic.CraftIronPick(s); ShowNotification("금광석 해금!"); });
            GUILayout.EndVertical();
        }
        else
        {
            GUILayout.BeginVertical("box");
            GUILayout.Label("< 미개척 구역 >");
            GUILayout.FlexibleSpace();
        ...
        string toolList = "";
        if (s.Upgrades.HasWoodPick) toolList += "[나무 곡괭이] ";
        if (s.Upgrades.HasIronPick) toolList += "[철 곡괭이] ";
        if (s.Upgrades.HasShears) toolList += "[원예 가위]";
